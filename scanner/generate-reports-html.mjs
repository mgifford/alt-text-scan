...
/**
 * Deduplicate reports by issue number, keeping only the most recent per issue.
 * Reports must already be sorted by time (most recent first) before calling this.
 * @param {Array<{path: string, data: object}>} reports - Reports sorted most recent first
 * @returns {Array<{path: string, data: object}>} Deduplicated reports (one per issue)
 */
...
export function generateReportsHtml(reports) {
  ...
  const deduplicatedAltTextReports = deduplicateReports(altTextReports);
  const legacyCount = legacyReports.length;
  const legacyNote = legacyCount > 0
    ? `<p class="section-note">Showing ${deduplicatedAltTextReports.length} alt-text reports. ${legacyCount} legacy accessibility reports are excluded from this page. <a href="legacy-reports.html">Browse the legacy archive</a>.</p>`
    : '';
  ...
  return generateReportsPage({
    reports: deduplicatedAltTextReports,
...
export function main() {
  ...
  console.log(`Generated ${outputFile} with ${altTextReports.length} alt-text reports (deduplicated to latest per issue)`);
